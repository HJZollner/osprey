...
t = resBasisSet.t;
for ii=1:nBasisFcts
    resBasisSet.fids(:,ii) = resBasisSet.fids(:,ii) .* exp(-1i*freqShift(ii).*t)' .* exp(-lorentzLB(ii).*t)';    
...
asym_factor = 2*gaussLB ./ (1 + exp(asym .* ppm_ax));
lineShape_function = sqrt(4*(log(2)/pi)) .* exp(- (4 * log(2) .* (ppm_ax ./ asym_factor)));

A = real(resBasisSet.specs);
for kk = 1:nBasisFcts
    A(:,kk) = A(:,kk) .* lineShape_function;
end
